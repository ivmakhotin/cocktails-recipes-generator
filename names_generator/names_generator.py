...
MAX_LENGTH = 51
MODEL_PATH = "names_generator_model.torch"
NUM_TOKENS = 51
EMBEDDING_SIZE = 32
RNN_NUM_UNITS = 512
...
for idx, (id, cocktail_data) in enumerate(mixology_cocktails.items()):
    lines.append(" " + cocktail_data["name"].lower())
...
#scocktail_name = 'mary-pickford-3822'
...
def gen_names():
    ...
    response["names"] = list()
    for _ in range(n):
        response["names"].append(generate_sample(model, init_state, temperature = temp))
